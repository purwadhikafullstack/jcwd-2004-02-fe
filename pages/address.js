...
const Address = () => { 
    ...
    const provinceHandleChange =async(e) => {
        setAddAddress({...addAddress, [e.target.name]: e.target.value})
        let res = await axios.get(`${API_URL}/transaction/getCities/${e.target.value}`)
        setCity(res.data)
    } 
    ...
    const onSubmitAddAddress = async (e) => {
        ...
        e.preventDefault()
        try {
            await axios.post(`${API_URL}/transaction/addAddress`, {
    ...
    return (
        <div>
            ...
            <div className="px-[380px] pt-[50px] mb-24 h-full"> 
                <span className="font-bold text-xl text-purple-900">Alamat Pengiriman</span>  
                <form onSubmit={onSubmitAddAddress}>
                    ...
                    <span className="font-bold text-sm text-purple-900">Info Penerima</span> 
                    <div className="my-8 flex justify-between">
                        <div className="w-[250px]">
                            <Input name="firstname" onChange={addAddressHandleChange}>Nama Depan</Input>
                        </div>
                        <div className="w-[250px]">
                            <Input name="lastname" onChange={addAddressHandleChange}>Nama Belakang</Input>
                        </div>
                    </div> 
                    <div className="w-[300px]">
                        <Input name="phonenumber" onChange={addAddressHandleChange}>Nomor HP</Input>
                    </div> 
                    <div className="flex justify-between my-8">
                        <div>
                            <span className="text-xs text-purple-800 mb-1">Provinsi</span>
                            <Select name="province_id" onChange={provinceHandleChange} w="250px" className="w-[150px] mt-2"> 
                        ...
                        <div> 
                            <span className="text-xs text-purple-800 mb-1">Kota/Kabupaten</span>
                            <Select name="city_id" onChange={addAddressHandleChange} w="250px" className="w-[250px] mt-2"> 
                            {city.map((val, index) => {
                            return (
                                <option key={index} value={val.id}>
                                {val.name}
                    ...
                    <div className="my-8">
                        <Input name="address" onChange={addAddressHandleChange}>Alamat</Input>
                    </div> 
                    {/* <div> 
                    ...
                    <div className="flex justify-between">
                        <ButtonSecondary className="w-[240px] h-[40px]">Batalkan</ButtonSecondary> 
                        <ButtonPrimary className="w-[240px] h-[40px]">Simpan Alamat</ButtonPrimary>
                    </div>
                </form>
            </div> 
            <Footer/>
